...
{	
	...
	while( !queue_filenames.empty() )	
	{
		...
		if( input == NULL )
		{
			print_message_safe(print_mutex, thread_id, 
				"Impossible to open the file", large_filename);
			continue;	
		}
		...
		treat_occurrences_tag_grams(tags_to_data_file, accepted_tags, 
...
int main(int argc, char** argv)
{
	...
	for(unsigned i=0; i<nb_cores; ++i)
		threads.emplace_back( [&]{calcul_occurrences( i+1, 
			queue_filenames, tags_to_data, accepted_tags, nb_ngrams); } );
	for(auto& t: threads)
		t.join();
	threads.clear();
