	...
	unsigned nb_ngram)
{
	for(unsigned i=0; i<filenames.size(); ++i)
		if( !calcul_freq(filenames[i], path_to_output, total_match, total_volume, nb_ngram) )
...
int main(int argc, char** argv)
{
	...
	calcul_handler( filenames, path_to_output_files, total_match, total_volume, nb_ngram );
