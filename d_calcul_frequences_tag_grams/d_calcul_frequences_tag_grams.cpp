...
{	
	...
	treat_occurrences_tag_grams(-1, tags_to_data, 
	...
	vector<string>& accepted_tags, unsigned nb_ngrams, unsigned min_year_defined)
{
	for(unsigned i=0; i<filenames.size(); ++i)
		if( !calcul_occurrences(filenames[i], tags_to_data, accepted_tags, nb_ngrams, min_year_defined) )
...
int main(int argc, char** argv)
{
	...
	collect_filenames(filenames, path_to_gz_pos_files, ".gz");
	calcul_handler(filenames, tags_to_data, accepted_tags, nb_ngrams, min_year_defined);
